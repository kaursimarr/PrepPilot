...
def build_chunks(segments, book_id, title):
    ...
    for unit in units:
        ...
        if cur and count + b.words > MAX_WORDS:
            groups.append(cur)
            cur, count = [], 0
...
def is_frontmatter(text):
    return bool(re.search(
        r"\bcopyright\b|all rights reserved|click here for terms of use",
        text, re.I
    ))
...
def is_toc_like(text):
    return bool(
        re.search(r"(?:\bcontents\b|\btable of contents\b)", text, re.I)
        and len(re.findall(r"\b\d+\b", text)) >= 8
    )


def is_index_like(text):
    return bool(
        re.search(r"(?im)^\s*(?:index|subject index|general index)\s*$", text)
